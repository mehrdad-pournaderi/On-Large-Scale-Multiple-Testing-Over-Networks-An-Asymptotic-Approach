...
def run_simulation(n_experiment=100, n_nodes=5, alpha=0.2, seed=0):
    rng = np.random.default_rng(seed)

    mu = 1.25 * np.arange(1, n_nodes+1)
    nn_range = np.arange(2, 5.1, 0.5)
    ...
    axs[0].plot(nn_range, final_FDR_noco, '-s', label='No-com BH')
    axs[0].plot(nn_range, final_FDR_pool, '-d', label='Pooled BH')
    axs[0].plot(nn_range, final_FDR_prop, '-*', label='Prop matching')
    axs[0].plot(nn_range, final_FDR_seg, '-o', label='Greedy agg')
    axs[0].axhline(alpha, ls='--', label='Target FDR')
    axs[0].set_ylim(0, 1)
    axs[0].set_ylabel('FDR')
    axs[0].legend()

    axs[1].plot(nn_range, final_power_noco, '-s', label='No-com BH')
    axs[1].plot(nn_range, final_power_pool, '-d', label='Pooled BH')
    axs[1].plot(nn_range, final_power_prop, '-*', label='Prop matching')
    axs[1].plot(nn_range, final_power_seg, '-o', label='Greedy agg')
    axs[1].set_ylim(0, 1)
    axs[1].set_xlabel('η')
...
if __name__ == '__main__':
    run_simulation(n_experiment=100, n_nodes=5, alpha=0.2, seed=42)
